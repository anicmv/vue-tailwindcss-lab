<!--
Tailwind 样式笔记（BilibiliBar）：
- 仅当前文件实际使用的 class（去重后）：
  ...
  69) `w-full`：宽度 100%。
  70) `h-[155px]`：高度 155px。
  71) `overflow-hidden`：隐藏溢出内容。
  72) `absolute`：绝对定位。
  73) `inset-0`：上下左右偏移都为 0。
  74) `bg-cover`：背景图覆盖容器。
...
<template>
  ...
  <div class="min-h-screen text-sm text-white">
    <header class="px-6 h-16 fixed top-0 left-0 right-0 z-50">
      <div class="flex h-full items-center justify-between">
        ...
        <div class="group w-[500px] h-[40px] bg-white/90 rounded-lg hover:bg-white focus-within:bg-white transition-colors">
          <div class="flex h-full items-center justify-between text-black">
            <input class="w-[445px] border-0 outline-none ring-0 rounded-md mx-[5px] px-[8px] py-[5px] bg-transparent focus:bg-gray-300/40 focus:border-0 focus:outline-none focus:ring-0 transition-colors"
                   ...
                   title="收纳柜"
            />
            <div class="ml-[2px] mr-[6px] rounded-md p-[6px] bg-transparent hover:bg-gray-300/40 transition-colors">
              <svg
                  class="w-[17px] h-[17px]"
                  viewBox="0 0 17 17"
                  fill="none"
        ...
        <ul class="flex gap-4">
              ...
              <a href="#">
                <picture>
                  <img
                      :src="avatarImage"
                      alt="用户头像"
                      class="w-[38px] h-[38px] rounded-full object-cover border-2 border-white"
                  />
                </picture>
              </a>
            </div>
          ...
          <li>
            <a href="#" class="inline-flex flex-col items-center justify-center gap-1">
              <svg
                  class="w-[20px] h-[20px]"
                  viewBox="0 0 20 20"
                  fill="none"
    ...
    <div class="relative w-full h-[155px] overflow-hidden">
      <div
          class="absolute w-full h-full inset-0 bg-cover bg-center"
